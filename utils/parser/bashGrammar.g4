...
linux_command		: 	COMMAND space? (tag | space | command_data | VAR | VAL | BLOB)* space? (PIPE space? (linux_command | sed ))* space? redirect? SEMICOLON? space?;

function_call		: 	VAR space? (tag | space | command_data | VAR | VAL)* SEMICOLON? space?; 
